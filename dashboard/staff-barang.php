    ...
    if (isset($_POST['add_barang'])){
        ...
        if (!empty($_FILES['image']['name'])) {
            ...
            $fileName = uniqid('barang_') . '.' . $ext;
              $supabaseUrl = uploadToSupabase($_FILES['image']['tmp_name'], $fileName, 'uploads');
              ...
              $pdo -> prepare("INSERT INTO barang_temuan (nama_barang, deskripsi, category, lokasi_ditemukan, tanggal_ditemukan, type, image, id_petugas, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'open')") -> execute([$nama, $deskripsi, $category, $lokasi, $tanggal, $type, $imageName, $u['id']]);
